...
public final class ClassUtil {
    ...
    private static void addClasses(Class<?> clazz, List<Class<?>> classes, Class<?> exclude, Class<?> parent) {
        if (logger.isDebugEnabled()) {
            logger.trace("----");
            if (exclude != null) {
                logger.trace("exclude:" + exclude.getName());
            }
            if (parent != null) {
                logger.trace("parent:" + parent.getName());
            }
    ...
    @SneakyThrows
    private static void findAndAddClassesInPackageByJar(String packageName, URL url, String packageDirName, List<Class<?>> classes, Class<?> exclude, Class<?> parent) {
        // 定义一个JarFile，获取jar
    ...
    @SneakyThrows
    private static void findAndAddClassesInPackageByFile(String packageName, String packagePath, List<Class<?>> classes, Class<?> exclude, Class<?> parent) {
        // 获取此包的目录 建立一个File
            ...
            for (File file : dirFiles) {
                ...
                if (file.isDirectory()) {
                    ...
                    if (!className.endsWith(".class")) {
                        logger.trace(className + "is not a class");
                    } else {
                        className = className.substring(0, className.length() - 6);
                        if (logger.isDebugEnabled()) {
                            logger.trace(packageName + '.' + className);
                        }
                        // 添加到集合中去
                        addClasses(Class.forName(packageName + '.' + className), classes, exclude, parent);
                    }
    ...
    @SneakyThrows
    private static List<Class<?>> getAllSubClass(String packageName, Class<?> exclude, Class<?> parent) {
        // 第一个class类的集合
    ...
    public static List<Class<?>> getAllSubClassByStr(String pkg) {
        return getAllSubClass(pkg, null, null);
    }

    public static List<Class<?>> getAllSubClassByStr(String pkg, Class<?> exclude) {
        return getAllSubClass(pkg, exclude, null);
    }

    public static List<Class<?>> getAllSubClassByStr(String pkg, Class<?> exclude, Class<?> parent) {
        return getAllSubClass(pkg, exclude, parent);
    }

    public static List<Class<?>> getAllSubClass(Package pkg) {
        return getAllSubClass(pkg.getName(), null, null);
    }

    public static List<Class<?>> getAllSubClass(Package pkg, Class<?> exclude) {
        return getAllSubClass(pkg.getName(), exclude, null);
    }

    public static List<Class<?>> getAllSubClass(Package pkg, Class<?> exclude, Class<?> parent) {
        return getAllSubClass(pkg.getName(), exclude, parent);
    }

    public static List<Field> getAllField(Class<?>... clazzes) {
    ...
    public static File getClasspathFile(String fileName) throws UnsupportedEncodingException {
        ClassLoader classLoader = ClassUtil.class.getClassLoader();
        URL url = classLoader.getResource(fileName);
        Asserts.notNull(url, "找不到资源" + fileName);
        String filepath = URLDecoder.decode(url.getFile(), UTF8);
        File file = new File(filepath);
        Asserts.isTrue(file.exists(), "文件" + file.getAbsolutePath() + "不存在");
        return file;
    }
    ...
    public static Type[] getGenericTypeExceptions(Method method, int index) {
        ...
        for (Type type : exceptionTypes) {
            logger.trace(String.valueOf(type));
            if (type instanceof ParameterizedType && i == index) {
                ParameterizedType parameterizedType = (ParameterizedType) type;
                logger.trace(String.valueOf(parameterizedType.getRawType()));
                Type[] actualTypeArguments = parameterizedType.getActualTypeArguments();
                for (Type t : actualTypeArguments) {
                    logger.trace(String.valueOf(t));
                }
    ...
    public static Type[] getGenericTypeReturnType(Method method) {
        ...
        logger.trace(String.valueOf(returnType));
